...
///
/// Bootloaders may leave this block empty or supply invalid data. Accessors
/// validate it before exposing display information. Extension blocks do not
/// fit in the boot protocol and are not used here.
#[repr(C)]
...
impl EdidInfo {
    ...
    /// unsupported version. Bytes after the base block are ignored.
    ...
    /// dimensions. EDID's centimeter units limit the precision to 10 mm.
    pub fn physical_size_mm(&self) -> Option<(u16, u16)> {
        ...
        Some((u16::from(width_cm) * 10, u16::from(height_cm) * 10))
    }
    ...
    fn is_valid(&self) -> bool {
        ...
        self.bytes.starts_with(&HEADER)
            && self.bytes[VERSION_OFFSET] == 1
            && self.bytes[REVISION_OFFSET] <= 4
            && self
                .bytes
                .iter()
