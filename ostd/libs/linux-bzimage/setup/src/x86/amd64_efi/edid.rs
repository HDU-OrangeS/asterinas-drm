...
//! Copies display information while EFI boot services still own the buffer.

use linux_boot_params::{EDID_BASE_BLOCK_SIZE, EdidInfo};
...
fn read_protocol<P: Protocol>(
    ...
    // SAFETY: These private protocol types describe read-only firmware data.
    // No mode changes or driver disconnections occur while the guard is alive,
    // and this code only reads the buffer through shared access.
    let protocol = unsafe {
    ...
    if fields.edid.is_null() || (fields.size_of_edid as usize) < EDID_BASE_BLOCK_SIZE {
        return Ok(None);
    }

    // SAFETY: UEFI provides `size_of_edid` initialized, read-only bytes. No
    // firmware calls or output/driver changes occur between reading the fields
    // and this copy. The checks above cover the fixed-size, byte-aligned read.
    // Copy before closing the protocol or exiting boot services; do not retain
    // a firmware pointer in the boot parameters.
    let bytes = unsafe { fields.edid.cast::<[u8; EDID_BASE_BLOCK_SIZE]>().read() };
