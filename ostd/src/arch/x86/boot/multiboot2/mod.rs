...
mod test {
    ...
    fn multiboot2_framebuffer_requires_rgb_metadata() {
        ...
        // SAFETY: The builder provides an aligned, initialized boot information
        // structure that remains alive for every use of `info`.
    ...
    fn parse_framebuffer(
        ...
        let structure = Builder::new()
            .framebuffer(FramebufferTag::new(0x1000, pitch, 3, 2, 32, buffer_type))
            .build();
        // SAFETY: The builder provides an aligned, initialized boot information
        // structure that remains alive for every use of `info`.
        let info = unsafe { BootInformation::load(structure.as_ptr()) }.unwrap();
