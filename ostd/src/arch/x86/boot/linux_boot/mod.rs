...
impl ToEarlyBootInfo for BootParams {
    ...
    fn framebuffer_arg(&self) -> Option<BootloaderFramebufferArg> {
        ...
        let pitch = if screen.lfb_linelength == 0 {
...
mod test {
    ...
    use linux_boot_params::{BootE820Entry, EDID_BASE_BLOCK_SIZE, EdidInfo};
    ...
    fn boot_params_with_framebuffer() -> BootParams {
        ...
        // SAFETY: All fields except the E820 entry types admit zero. Initialize every
        // E820 entry with a valid enum discriminant before creating the `BootParams` value.
